...
#define EDWARDS25519_PRIVATE __attribute__((visibility("hidden")))
#else
#define EDWARDS25519_PRIVATE
#endif
...
void ge4x_from_ge25519(ge4x *r, const ge25519 *p)
{
	...
	/* Preserve the projective representation; packing the point itself would
	 * only retain affine y and the sign of x. */
	fe25519_pack(packed, &p->x);
	gfe4x_unpack_single(&r->x, packed, 0);
	fe25519_pack(packed, &p->y);
	gfe4x_unpack_single(&r->y, packed, 0);
	fe25519_pack(packed, &p->z);
	gfe4x_unpack_single(&r->z, packed, 0);
	fe25519_pack(packed, &p->t);
	gfe4x_unpack_single(&r->t, packed, 0);
	for (i = 0; i != 12; ++i) {
		r->x.v[i].v[1] = r->x.v[i].v[2] = r->x.v[i].v[3] = r->x.v[i].v[0];
		r->y.v[i].v[1] = r->y.v[i].v[2] = r->y.v[i].v[3] = r->y.v[i].v[0];
		r->z.v[i].v[1] = r->z.v[i].v[2] = r->z.v[i].v[3] = r->z.v[i].v[0];
		r->t.v[i].v[1] = r->t.v[i].v[2] = r->t.v[i].v[3] = r->t.v[i].v[0];
	}
}
...
EDWARDS25519_PRIVATE const gfe4x Gk = 
{{
	{ repeat4x(3338585.0) } ,
...
extern void ge4x_lookup_niels_asm(ge4x_niels *, const double (*)[3][12], const signed char *);

extern void ge4x_lookup_asm(ge4x *, const ge4x *, const signed char *);

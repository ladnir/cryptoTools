...
template <AESTypes type> class AES {
public:
  ...
  // Use this function (or other *Inline functions) if you want to force
  // inlining.
  template <u64 blocks>
  ...
  template <u64 blocks>
  OC_FORCEINLINE void ecbEncBlocksInline(const block (&plaintext)[blocks],
                                         block (&ciphertext)[blocks]) const {
  ...
  // Not necessarily inlined version. See specialization below class for more
  // information.
  template <u64 blocks>
  ...
  // Encrypts blockLength starting at the plaintext pointer and writes the
  // result to the ciphertext pointer
  inline void ecbEncBlocks(const block *plaintext, u64 blockLength,
...
#undef SWITCH_CASE
  ...
  inline void ecbEncBlocks(span<const block> plaintext,
                           span<block> ciphertext) const {
  ...
  // Encrypts the vector of blocks {baseIdx, baseIdx + 1, ..., baseIdx +
  // blockLength - 1} and writes the result to ciphertext.
  void ecbEncCounterMode(u64 baseIdx, u64 blockLength,
                         block *ciphertext) const {
  ...
  void ecbEncCounterMode(block baseIdx, u64 blockLength,
                         block *ciphertext) const {
  ...
  OC_FORCEINLINE typename std::enable_if<(blocks <= 16)>::type
  TmmoHashBlocks(const block *plaintext, block *ciphertext,
                 TweakFn &&tweakFn) const {
    block buff[blocks];
    block pix[blocks];
  ...
  inline void TmmoHashBlocks(span<const block> plaintext,
                             span<block> ciphertext, TweakFn &&tweak) const {
    if (plaintext.size() != ciphertext.size())
      throw RTE_LOC;

    TmmoHashBlocks(plaintext.data(), plaintext.size(), ciphertext.data(),
                   tweak);
  }
  ...
  // y_i = AES(AES(x_i) ^ tweak_i) + AES(x_i).
  template <typename TweakFn>
  inline void TmmoHashBlocks(const block *plaintext, u64 blockLength,
                             block *ciphertext, TweakFn &&tweak) const {
    const u64 step = 8;
    u64 idx = 0;
...
#undef SWITCH_CASE
  ...
  // Correlation robust hash function.
  // H(x) = AES(x) + x.
  inline void hashBlocks(span<const block> plaintext,
                         span<block> ciphertext) const {
    if (plaintext.size() != ciphertext.size())
      throw RTE_LOC;
  ...
  // Correlation robust hash function.
  // H(x) = AES(x) + x.
  inline void hashBlocks(const block *plaintext, u64 blockLength,
                         block *ciphertext) const {
    const u64 step = 8;
    u64 idx = 0;
...
private:
  ...
  // For simplicity, all CTR modes are defined in terms of the unaligned
  // version.
  void ecbEncCounterModeImpl(block baseIdx, u64 blockLength,
                             u8 *ciphertext) const;
  ...
  xorBlocks(const block *plaintext, block *buff, block *ciphertext) {
    ...
    // Only write to ciphertext after computing the entire output, so the
    // compiler won't have to worry about ciphertext aliasing plaintext.
...
template <> void AES<NI>::setKey(const block &userKey);

template <> inline block AES<NI>::finalEnc(block state, const block &roundKey) {
  return _mm_aesenclast_si128(state, roundKey);
}

template <> inline block AES<NI>::roundEnc(block state, const block &roundKey) {
...
template <AESTypes type> class AESDec {
public:
  ...
  AESDec(const block &key) { setKey(key); }
...
// different keys
template <int N> class MultiKeyAES {
public:
  ...
  // TODO: use technique from "Fast Garbling of Circuits Under Standard
  // Assumptions".
  template <int SS> void keyGenHelper8(std::array<block, 8> &key) {
...
struct AESStream {
  ...
  AESStream(AESStream &&o) noexcept
      : mPrng(std::move(o.mPrng)), mAesRoundKeys(std::move(o.mAesRoundKeys)),
  ...
  AESStream(block seed) { setSeed(seed); }

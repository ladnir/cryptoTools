    ...
    void IOService::receiveOne(ChannelBase* channel)
    {
        ...
        if (op.type() == IOOperation::Type::RecvData)
        {
            ...
            channel->mHandle->async_recv(tt, [&op, channel, this](const boost::system::error_code& ec, u64 bytesTransfered) 
            {
                ...
                {
                    std::string reason = "Network receive error: ";

                    if (ec) reason += ec.message() + "\n";
                    else reason += "incorrect bytes received, " + ToString(bytesTransfered) + " but expecting " + ToString(boost::asio::buffer_size(op.mBuffs[0])) + "\n";

                    reason += "Channel: " + channel->mLocalName
                        + ", Session: " + channel->mSession->mName + " " + ToString(channel->mSession->mPort) + " "
                        + (channel->mSession->mMode == SessionMode::Server ? "server" : "client");
                ...
                if (channel->mRecvSizeBuff != op.size() && op.resize(channel->mRecvSizeBuff) == false)
                    msg += "Network receive error: Expecting to receive " + ToString(op.size()) + " bytes but actaully received " + std::to_string(channel->mRecvSizeBuff) + " bytes. Resize attempted.";
                ...
                auto recvMain = [&op, channel, this](const boost::system::error_code& ec, u64 bytesTransfered)
                {
                    ...
                    {
                        std::string reason = "Network receive error (2): ";

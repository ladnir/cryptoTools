    ...
    bool simple, bool encrypto, bool rand, u64 seed)
{
    ...
    auto routine = [tries, verbose, &counts, setSize, h, e, numThrds, rand, simple, encrypto, seed](u64 tIdx)
    {
        ...
        for (u64 i = 0; i < count; ++i)
        {
            ...
            else
            {
                ...
                if (stashSize && verbose)
                {

                    for (u64 i = 0; i < c.mLocations.rows(); ++i)
                    {
                        auto h = block(
                            std::array<u32, 4>{
                                (u32)c.mLocations(i, 0),
                                (u32)c.mLocations(i, 1),
                                (u32)c.mLocations(i,2),
                                0
                        });
                        std::cout << i << " { " << h.get<u32>(0) << ", "
                            << h.get<u32>(1) << ", "
                            << h.get<u32>(2) << "}"<<std::endl;
                    }

                    std::cout << "ss " << stashSize << std::endl;
                }
            }
    ...
    while (n <= nEnd)
    {
        ...
        while (eStep > 0 ? curE <= eEnd : curE >= eEnd)
        {
            ...
            runOne(curSetSize, h, curE,
                ...
                stashSize, out,verbose, simple, encrypto, rand, seed);
            curE += eStep;
        }

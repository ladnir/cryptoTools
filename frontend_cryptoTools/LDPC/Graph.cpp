...
//#endif
    ...
    void LDPC::blockTriangulate(std::vector<u64>& R, std::vector<u64>& C, bool verbose)
    {
        ...
        std::vector<btree::set<u64>> bigWeightSets;
    ...
    void ldpc(CLP& cmd)
    {
        ...
        for (u64 i = 0; i < t; ++i)
        {
...

            {
                ...
                auto start = timer.setTimePoint("");
                H2.blockTriangulate(R, C, v, stats);
                auto end = timer.setTimePoint("");
                ...
                if (v)
                {
                    std::cout << LDPC2::diff(H2, H2, R, C) << std::endl;
                    std::cout << "--------------------------------" << std::endl;
                }
            }
        }

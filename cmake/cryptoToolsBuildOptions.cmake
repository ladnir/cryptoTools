...
option(ENABLE_VAES      "statically use 256-bit VAES for batched AES (requires VAES CPU)" OFF)
...
if(ENABLE_VAES)
	if(NOT CMAKE_SYSTEM_PROCESSOR MATCHES "^(x86_64|AMD64|amd64|i.86|x86)$" OR
	   NOT ENABLE_SSE OR NOT ENABLE_AVX OR NOT ENABLE_AVX2 OR ENABLE_ARM_AES)
		message(FATAL_ERROR "ENABLE_VAES requires x86, ENABLE_SSE, ENABLE_AVX and ENABLE_AVX2, without ARM AES")
	endif()
	include(CheckCXXSourceCompiles)
	set(_oc_vaes_required_flags "${CMAKE_REQUIRED_FLAGS}")
	if(MSVC)
		set(CMAKE_REQUIRED_FLAGS "${CMAKE_REQUIRED_FLAGS} /arch:AVX2")
	else()
		set(CMAKE_REQUIRED_FLAGS "${CMAKE_REQUIRED_FLAGS} -mavx2 -mvaes")
	endif()
	check_cxx_source_compiles("#include <immintrin.h>\nint main(){ __m256i x=_mm256_setzero_si256(); x=_mm256_aesenc_epi128(x,x); x=_mm256_aesenclast_epi128(x,x); return _mm256_extract_epi32(x,0); }" OC_COMPILER_HAS_VAES)
	set(CMAKE_REQUIRED_FLAGS "${_oc_vaes_required_flags}")
	unset(_oc_vaes_required_flags)
	if(NOT OC_COMPILER_HAS_VAES)
		message(FATAL_ERROR "ENABLE_VAES requires compiler support for 256-bit VAES intrinsics")
	endif()
endif()
...
message(STATUS "Option: ENABLE_VAES         = ${ENABLE_VAES}")
